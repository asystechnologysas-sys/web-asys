...
export async function generateWithProvider({ messages, context, language, pageContext = '' }) {
  ...
  const response = await fetch(OPENAI_API_URL, {
    ...
    body: JSON.stringify({ model: process.env.OPENAI_MODEL || 'gpt-4o-mini', temperature: 0.25, max_tokens: 420, messages: [{ role: 'system', content: `${system}\nContexto de navegación: ${pageContext || 'inicio'}` }, ...messages] })
  });
